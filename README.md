...
`constructor(cdn: string, baseUrl: string, options?: RequestInit)` - конструктор принимает базовый url API сервера и адрес для CDN, а так же необязательные дополнительыне данные для запросов;
...
### 3.Класс Modal - класс, расширяющий базовый класс Component. Отвечает за управление модальным окном:

###### Конструктор: 

` constructor(container: HTMLElement, protected events: IEvents)` - конструктор принимает DOM-элемент модального окна и экземпляр `EventEmitter`;
...
`set content(value: HTMLElement)` - сеттер для установки нового содержимого модального окна. Принимает: элемент для установки.

`open()` - открывает модальное окно.

`close()` - закрывает модальное окно.

`render(data: IModalData): HTMLElement` - рендерит и открывает модальное окно. Принимает: объект с контентом для отображения. Возвращает: HTMLElement модального окна.

***


### 4.Класс Form - универсальный класс, расширяющий базовый класс Component. Отвечает за управление формами:

###### Конструктор: 

` constructor(protected container: HTMLFormElement, protected events: IEvents)` - конструктор принимает элемент формы и экземпляр `EventEmitter`;
...
`onInputChange(field: keyof T, value: string)` - уведомляет об изменении данных инпута. Принимает: ключ поля формы и новое значение поля.

`set valid(value: boolean)` - меняет статус активности кнопки. Принимает: логическое значение true/false валидности формы.

`set errors(value: string)` - отображает текст ошибок валидации. Принимает: текст ошибки.

`render(state: Partial<T> & IFormState)` - обновляет внешний вид формы. Принимает: значение валидности, ошибок и остальной информации формы.

***


### 5.Классы Order и Contacts - классы, расширяющие класс Form. Отвечают за формы заказов:
...
`_paymentBtn: HTMLButtonElement[]` - массив HTML-элементов, представляющих собой кнопки выбора способа оплаты.

###### Методы класса Order: 

`set address(value: string) ` - устанавливает значение поля адреса. Принимает: строку с адресом доставки.

`set payment(name: string) ` - активирует кнопку (меняет визуальное состояние) исходя из выбранного способа оплаты. Принимает: строку, которая является именем выбранного способа оплаты.

###### Методы класса Contacts: 

`set phone(value: string)` - устанавливает значение поля номера телефона. Принимает: строку с номером телефона.

`set email(value: string)` - устанавливает значение поля электронной почты. Принимает: строку с адресом почты.

***

### 6.Класс Basket - класс, расширяющий базовый класс Component. Отвечает за отображение корзины:

###### Конструктор: 

`constructor(container: HTMLElement, protected events: EventEmitter)` - принимает DOM-элемент корзины и экземпляр `EventEmitter`;

###### Поля класса:

`_list: HTMLElement` - список товаров.

`_total: HTMLElement` - общая цена товаров корзины.

`_button: HTMLElement` - кнопка перехода к оформлепнию заказа.
...
`set total(total: number)` - устанавливает итоговую стоимость товаров корзины. Принимает: значение числовое общей стоимости.

***


### 7.Класс Success - класс, расширяющий базовый класс Component. Отвечает за отображение сообщения об успешном заказе:

###### Конструктор: 

` constructor(container: HTMLElement, actions: ISuccessActions) ` - принимает DOM-элемент и обработчик клика по кнопке;

###### Поля класса:

`_close: HTMLElement` - кнопка закрытия окна.

`_total: HTMLElement` - общая стоимость списания.


###### Методы класса: 

`set total(total: number)` - обновляет текст итогового списания. Принимает: значение числовое общей стоимости.
...
Код презентера не вынесен в отдельный класс, а размещен в основном скрипте приложения index.ts, поскольку в приложении всего одна страница и этого достаточно для управления всеми компонентами. Презентер подписывается на события от Представления, обрабатывает их и вызывает методы Модели, а также подписывается на события от Модели и обновляет Представление.

***

# События 

#### В проекте используется событийно-ориентированный подход, где все компоненты взаимодействуют через EventEmitter. События можно разделить на две основные категории: события изменения данных и события пользовательского интерфейса.

### Cобытия изменения данных:
...
### Cобытия пользовательского интерфейса:
...
### Пример взаимодействия(пользователь кликает по карточке товара):

1. Презентер ждет, когда пользователь кликнет на карточку товара - будет событие `card:select`.
   
2. Когда пользователь кликнул, он передает команду в `AppData` сохранить информацию для превью.

3. Модель генерирует событие `preview:changed`.
   
4. Презентер видит эт о событие и готовит новый экземпляр `Card` для модального окна, заполняет данными и настраивает кнопку.
   
5. В конце презентер передает экземпляр карточки в модальное окно для отображения.
