...
`_submit: HTMLButtonElement` - кнопка закрытия модального окна.

`_errors: HTMLElement` - контейнер содержимого.
...
`set address(value: string) ` - уведомляет об изменении данных инпута. Принимает: ключ поля формы и новое значение поля.

`set address(value: string) ` - меняет статус активности кнопки. Принимает: логическое значение true/false валидности формы.

###### Методы класса Contacts: 

`set phone(value: string)` - уведомляет об изменении данных инпута. Принимает: ключ поля формы и новое значение поля.

`set email(value: string)` - меняет статус активности кнопки. Принимает: логическое значение true/false валидности формы.
