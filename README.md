...
Код презентера не вынесен в отдельный класс, а размещен в основном скрипте приложения index.ts, поскольку в приложении всего одна страница и этого достаточно для управления всеми компонентами. Презентер подписывается на события от View, обрабатывает их и вызывает методы Модели, а также подписывается на события от Модели и обновляет View.
